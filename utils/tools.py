...
def vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    ...
    model.eval()
    with torch.no_grad():
        for i, (batch_x, _, _, batch_y, batch_y_mask) in tqdm(enumerate(vali_loader)):
            batch_x = batch_x.float().to(accelerator.device)
            batch_y = batch_y.float()
            batch_y_mask = batch_y_mask.bool()
            ...
            mae_loss = (mae_loss*batch_y_mask.float()).sum()
            mae_loss = mae_loss/batch_y_mask.sum()
