...
# forecasting task
parser.add_argument('--seq_len', type=int, default=46, help='input sequence length')
parser.add_argument('--label_len', type=int, default=23, help='start token length')
parser.add_argument('--pred_len', type=int, default=46, help='prediction sequence length')
...
for ii in range(args.itr):
    ...
    for epoch in range(args.train_epochs):
        ...
        for i, (batch_x, _, _, batch_y, batch_y_mask) in tqdm(enumerate(train_loader)):
            ...
            batch_y_mask = batch_y_mask.float().to(accelerator.device)

            # decoder input
            dec_inp = torch.zeros_like(batch_y[:, -args.pred_len:, :]).float().to(
                accelerator.device)
            dec_inp = torch.cat([batch_x[:, -args.label_len:, :], dec_inp], dim=1).float().to(
                accelerator.device)
            ...
            if args.use_amp:
                with torch.cuda.amp.autocast():
                    if args.output_attention:
                        outputs = model(x_enc=batch_x, x_dec=dec_inp)[0]
                    else:
                        outputs = model(x_enc=batch_x, x_dec=dec_inp)

                    f_dim = -1 if args.features == 'MS' else 0
                    outputs = outputs[:, -args.pred_len:, f_dim:]
                    batch_y = batch_y[:, -args.pred_len:, f_dim:].to(accelerator.device)
                    loss = criterion(outputs, batch_y)
                    loss = (loss*batch_y_mask.float()).sum()
